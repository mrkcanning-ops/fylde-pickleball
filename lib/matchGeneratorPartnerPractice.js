...
export function generatePartnerPracticeRandom(players, numCourts = 2) {
  ...
  // Generate multiple rounds based on player count (minimum 2 rounds)
  const numRounds = Math.max(2, Math.ceil(activePlayers.length / 4));

  for (let round = 0; round < numRounds; round++) {
    // Shuffle players differently each round
...
export function generatePartnerPracticeGenderDoubles(players, numCourts = 2) {
  ...
  for (let round = 0; round < numRounds; round++) {
    // Re-shuffle for each round for variety
    let malesPaired = activePlayers.filter(p => p.gender === 'male' && pairedPlayers.has(p.id)).sort(() => Math.random() - 0.5);
    let femalesPaired = activePlayers.filter(p => p.gender === 'female' && pairedPlayers.has(p.id)).sort(() => Math.random() - 0.5);
    let malesUnpaired = activePlayers.filter(p => p.gender === 'male' && !pairedPlayers.has(p.id)).sort(() => Math.random() - 0.5);
    let femalesUnpaired = activePlayers.filter(p => p.gender === 'female' && !pairedPlayers.has(p.id)).sort(() => Math.random() - 0.5);
    ...
    while (malesPaired.length >= 2 && matchCount < numMatches) {
      ...
      allMatches.push([[p1, partner1], [p2, partner2]]);
      matchCount++;
    }
    ...
    // Fill remaining matches ONLY with same-gender combinations
    // Separate remaining players by gender
    const remainingMales = [...malesPaired, ...malesUnpaired];
    const remainingFemales = [...femalesPaired, ...femalesUnpaired];

    // Create male-only matches from remaining males
    while (remainingMales.length >= 4 && matchCount < numMatches) {
      const team1 = [];
      const team2 = [];

      // Try to keep male pairs together
      for (let i = 0; i < remainingMales.length && team1.length < 2; i++) {
        const player = remainingMales[i];
        const partner = remainingMales.find(p => pairs.get(player.id) === p.id);
        
        if (partner) {
          team1.push(player);
          team1.push(partner);
          remainingMales.splice(remainingMales.indexOf(player), 1);
          remainingMales.splice(remainingMales.indexOf(partner), 1);
          break;
        }
      }

      while (team1.length < 2 && remainingMales.length > 0) {
        team1.push(remainingMales.shift());
      }
      while (team2.length < 2 && remainingMales.length > 0) {
        team2.push(remainingMales.shift());
      }
    ...
    // Create female-only matches from remaining females
    while (remainingFemales.length >= 4 && matchCount < numMatches) {
      const team1 = [];
      const team2 = [];

      // Try to keep female pairs together
      for (let i = 0; i < remainingFemales.length && team1.length < 2; i++) {
        const player = remainingFemales[i];
        const partner = remainingFemales.find(p => pairs.get(player.id) === p.id);
        
        if (partner) {
          team1.push(player);
          team1.push(partner);
          remainingFemales.splice(remainingFemales.indexOf(player), 1);
          remainingFemales.splice(remainingFemales.indexOf(partner), 1);
          break;
        }
      }

      while (team1.length < 2 && remainingFemales.length > 0) {
        team1.push(remainingFemales.shift());
      }
      while (team2.length < 2 && remainingFemales.length > 0) {
        team2.push(remainingFemales.shift());
      }
...
export function generatePartnerPracticeGenderMixed(players, numCourts = 2) {
  ...
  for (let round = 0; round < numRounds; round++) {
    // Shuffle for variety each round
    males = males.sort(() => Math.random() - 0.5);
    ...
    for (const { male, female } of malePartners) {
      ...
      if (remainingMales.length >= 1 && remainingFemales.length >= 1) {
        ...
        if (male2 && female2) {
          ...
          allMatches.push([[male, female], [male2, female2]]);
          matchCount++;
        }
      }
    }
    ...
    let remainingMales = males.filter(m => !usedMales.has(m.id));
    let remainingFemales = females.filter(f => !usedFemales.has(f.id));
    ...
    while (remainingMales.length >= 2 && remainingFemales.length >= 2 && matchCount < numMatches) {
      ...
      if (m1 && m2 && f1 && f2) {
        allMatches.push([[m1, f1], [m2, f2]]);
        matchCount++;
      }
    }

    // Handle remaining players if any
    remainingMales = males.filter(m => !usedMales.has(m.id));
    remainingFemales = females.filter(f => !usedFemales.has(f.id));
