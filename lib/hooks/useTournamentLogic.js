...
export function useTournamentLogic() {
  ...
  const initializeTournament = useCallback(
    (players, format = 'single-elimination', gameType = 'singles', courtsCount = 2) => {
      ...
      if (format === 'group-knockout') {
        result = generateGroupKnockoutBracket(players, gameType);
      } else if (format === 'double-elimination') {
        result = generateDoubleEliminationBracket(players, gameType);
      } else {
        result = generateSingleEliminationBracket(players, gameType);
      }
      ...
      // Add courts count to bracket and assign court numbers
  ...
  const advanceToNextRound = useCallback(() => {
    ...
    if (currentBracket.format === 'group-knockout' && currentBracket.stage === 'group') {
