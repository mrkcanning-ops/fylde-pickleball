...
export function useTournamentLogic() {
  ...
  const advanceToNextRound = useCallback((bracketToUse = null) => {
    ...
    if (bracket.stage === 'knockout' && lastRound.matchups.length === 1 && lastRound.matchups[0].played) {
    ...
    const result = bracket.stage === 'knockout'
      ? generateNextKnockoutRound(bracket.knockoutRounds, bracket.knockoutRounds.length - 1)
  ...
  const transitionToKnockout = useCallback(() => {
    ...
    const allGroupsComplete = groupRounds.every(round =>
