...
export function generateRoundRobinMatches(players, numCourts = 2) {
  ...
  // Convert round-based match groups into court arrays while randomly assigning
  // matches to courts each round to mix up which players are on which court
  const courtMatches = Array.from({ length: numCourts }, () => []);
  rounds.forEach((roundSet) => {
    // Randomly shuffle which court each match is assigned to
    const courtIndices = Array.from({ length: roundSet.length }, (_, i) => i);
    for (let i = courtIndices.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [courtIndices[i], courtIndices[j]] = [courtIndices[j], courtIndices[i]];
    }
    
    roundSet.forEach((match, matchIndex) => {
      const courtIndex = courtIndices[matchIndex];
