...
export function generateRoundRobinMatches(players, numCourts = 2) {
  ...
  // Target config based on player count
  const getRoundConfig = (playerCount) => {
    if (playerCount === 4) return { rounds: 3, gamesPerPlayer: 3, byesPerPlayer: 0 };
    if (playerCount === 5) return { rounds: 5, gamesPerPlayer: 4, byesPerPlayer: 1 };
    if (playerCount === 6) return { rounds: 6, gamesPerPlayer: 4, byesPerPlayer: 2 };
    if (playerCount === 7) return { rounds: 7, gamesPerPlayer: 4, byesPerPlayer: 3 };
    if (playerCount === 8) return { rounds: 6, gamesPerPlayer: 3, byesPerPlayer: 3 };
    return { rounds: Math.ceil(playerCount / 2), gamesPerPlayer: Math.floor((playerCount - 1) / 2), byesPerPlayer: Math.ceil((playerCount - 1) / 2) };
  };

  const config = getRoundConfig(n);
  const targetRounds = config.rounds;
  const targetGamesPerPlayer = config.gamesPerPlayer;
  const targetByesPerPlayer = config.byesPerPlayer;
      ...
      for (let j = i + 1; j < n - 2; j++) {
        for (let k = j + 1; k < n - 1; k++) {
          for (let l = k + 1; l < n; l++) {
            ...
            const resting = activePlayers.filter((p) => !quartet.some((q) => q.id === p.id));
            ...
            for (const [team1, team2] of teamOptions) {
              ...
              // Score: prioritize new partnerships, then fairness
              const score = newPartners * 100 - repeatPartners * 40 - gamesSpread * 8 - restSpread * 6;
