...
export function generatePartnerPracticeMatches(players, numCourts = 2) {
  ...
  let round = 0;
  const maxRounds = Math.ceil(n * 1.2); // Allow more rounds for convergence
  const targetGamesPerPlayer = Math.ceil(n * 0.7);
  let consecutiveFailures = 0;

  while (round < maxRounds && consecutiveFailures < 5) {
    let bestPlan = null;
    ...
    // Optimization: try combinations - increase search space for better results
    const searchLimit = n > 10 ? 2000 : n > 8 ? 1000 : 10000;
    let iterations = 0;
      ...
      for (let j = i + 1; j < n - 2 && iterations < searchLimit; j++) {
        for (let k = j + 1; k < n - 1 && iterations < searchLimit; k++) {
          for (let l = k + 1; l < n && iterations < searchLimit; l++) {
            ...
            // Check rest distribution cap - allow higher threshold for larger groups
            const restCap = Math.ceil(n * 2);
            if (resting.some((p) => restCounts[p.id] >= restCap)) continue;
            ...
            for (const [team1, team2] of teamOptions) {
              ...
              // 4. Penalize resting unpaired players who need games (but allow it as last resort)
              const unpairedNeedingGamesResting = unpairedResting.filter((p) => gamesPlayed[p.id] < targetGamesPerPlayer);
              const penaltyForRestingUnpaired = unpairedNeedingGamesResting.length * 5;

              // Prioritize: unpaired players playing > partner bonus > balance > rest unpaired
              const score = unpairedInQuartetScore * 10 + partnerBonusScore - penaltyForRestingUnpaired - gamesSpread * 2 - restSpread * 2;
