...
export function generatePartnerPracticeMatches(players, numCourts = 2) {
  ...
  // Generate matches
  let round = 0;
  const maxRounds = Math.ceil(n * 2); // Allow generous rounds
  ...
  while (round < maxRounds) {
      ...
      for (let j = i + 1; j < n - 2; j++) {
        for (let k = j + 1; k < n - 1; k++) {
          for (let l = k + 1; l < n; l++) {
            ...
            const pairedInQuartet = quartet.filter((p) => pairedPlayers.has(p.id));
            const pairedResting = resting.filter((p) => pairedPlayers.has(p.id));
    ...
    if (!bestPlan) break; // No valid matches found
