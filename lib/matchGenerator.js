...
export function generateRoundRobinMatches(players, numCourts = 2) {
  ...
  rounds.forEach((roundSet, roundIdx) => {
    ...
    // Score each possible assignment and pick the best one
    let bestAssignment = null;
    let bestScore = -Infinity;
    
    // For 2 courts with 2 matches, try both possible court assignments
    const possibleAssignments = [
      [0, 1], // match 0 on court 0, match 1 on court 1
      [1, 0], // match 0 on court 1, match 1 on court 0
    ];
    ...
    // If tie or no history, use assignment with some randomness
    if (!bestAssignment) {
      bestAssignment = Math.random() < 0.5 ? [0, 1] : [1, 0];
    }
