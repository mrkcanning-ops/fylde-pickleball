...
export function assignCourtsToRounds(bracket, courtsCount = 2) {
  ...
  updatedBracket.rounds.forEach((round) => {
    ...
    if (round.matchups) {
      round.matchups.forEach((match, index) => {
        match.court = courtIndex;
        courtIndex += 1;
        if (courtIndex > courtsCount) {
          courtIndex = 1; // Wrap around to court 1
        }
      });
    }
  });

  // Assign courts to knockout rounds if they exist
  ...
  bracket.rounds.forEach((round) => {
    if (round.bracketType === 'group') {
      ...
      (round.matchups || []).forEach((match) => {
        ...
        const isDoubles = match.team1 && match.team1.length > 1;
...
export function generateGroupKnockoutBracket(players = [], gameType = 'singles') {
  ...
  const groupRounds = groups.map((group, groupIdx) => {
    ...
    if (gameType === 'doubles') {
      ...
      // Create teams by pairing consecutive players
      const teams = [];
      for (let i = 0; i < groupPlayers.length; i += 2) {
        teams.push([groupPlayers[i], groupPlayers[i + 1]]);
      }
