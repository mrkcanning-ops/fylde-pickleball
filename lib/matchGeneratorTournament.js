...
export function generateGroupKnockoutBracket(players = [], gameType = 'singles') {
  ...
  // Divide into groups (serpentine seeding)
  const numGroups = seeded.length <= 8 ? 2 : 3;
    ...
    for (let i = 0; i < group.length; i++) {
      for (let j = i + 1; j < group.length; j++) {
        matchups.push({
          ...
          team1: gameType === 'doubles' ? [group[i], group[i + 1]] : [group[i]],
          team2: gameType === 'doubles' ? [group[j], group[j + 1]] : [group[j]],
          winner: null,
          played: false,
          groupId: groupIdx + 1,
        });
  ...
  return {
    ...
    bracket: {
      ...
      playerCount: activePlayers.length,
      numGroups,
      totalRounds: Math.ceil(Math.log2(Math.ceil(activePlayers.length / numGroups) * 2)) + 1, // groups + knockout
      seededPlayers: seeded,
