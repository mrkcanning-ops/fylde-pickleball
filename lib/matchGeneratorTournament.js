 ...
 * Doubles: minimum 4 players (even number for full pairs)
 */
function validatePlayerCountForBracket(playerCount, gameType) {
  if (gameType === 'doubles') {
    if (playerCount < 4) {
      return {
        valid: false,
        error: `Doubles requires at least 4 players (2 pairs). You have ${playerCount} players.`,
      };
    }
    if (playerCount % 2 !== 0) {
      return {
        valid: false,
        error: `Doubles requires an even number of players (full pairs). You have ${playerCount} players.`,
      };
    }
  } else {
    if (playerCount < 2) {
      return {
        valid: false,
        error: `Singles requires at least 2 players. You have ${playerCount} players.`,
      };
    }
  }
 ...
 * Doubles: minimum 8 players (4 pairs for 2 groups of 2 pairs), even number
 */
function validatePlayerCountForGroupStage(playerCount, gameType) {
  if (gameType === 'doubles') {
    if (playerCount < 8) {
      return {
        valid: false,
        error: `Doubles group stage requires minimum 8 players (4 pairs). You have ${playerCount} players.`,
      };
    }
    if (playerCount % 2 !== 0) {
      return {
        valid: false,
        error: `Doubles group stage requires an even number of players (full pairs). You have ${playerCount} players.`,
      };
    }
  } else {
    if (playerCount < 4) {
      return {
        valid: false,
        error: `Singles group stage requires minimum 4 players (2 groups of 2). You have ${playerCount} players.`,
      };
    }
  }
  return { valid: true, error: null };
...
export function generateKnockoutFromGroups(bracket) {
  ...
  return {
    roundNumber: 1,
    stageName: matchups.length === 2 ? 'Semifinals' : matchups.length === 4 ? 'Quarterfinals' : 'Round of ' + matchups.length * 2,
    bracketType: 'knockout',
    matchups,
  };
}
...
export function generateGroupKnockoutBracket(players = [], gameType = 'singles', doublesConfig = {}) {
  ...
  // Determine number of groups: 4 groups for 16+ players, 2 groups for fewer players
  const numGroups = activePlayers.length >= 16 ? 4 : 2;
