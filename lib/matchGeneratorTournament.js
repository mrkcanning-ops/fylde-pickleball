...
export function generateNextKnockoutRound(knockoutRounds, currentRoundIndex) {
  ...
  const winners = matchups.filter(m => m.winner).map(m => m.winner);
  const losers = matchups
    .filter(m => m.winner) // Only losers from matches that have been played
    .map((m, idx) => {
      // Determine who lost in this match
      if (Array.isArray(m.winner)) {
        // For doubles, find who lost
        return m.team1?.some(p => m.winner.some(wp => wp.id === p.id)) ? m.team2 : m.team1;
      } else {
        // For singles
        return m.winner.id === m.team1?.[0]?.id ? m.team2 : m.team1;
      }
    })
    .filter(Boolean);
