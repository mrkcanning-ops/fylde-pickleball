...
export function assignCourtsToRounds(bracket, courtsCount = 2) {
  ...
  // Process group rounds
  updatedBracket.rounds.forEach((round) => {
    if (!round.matchups || round.matchups.length === 0) return;

    // Track players assigned to each court in this round
    const courtPlayers = {};
    for (let i = 1; i <= courtsCount; i++) {
      courtPlayers[i] = new Set();
    }

    // Assign matches to courts, respecting player availability
    round.matchups.forEach((match) => {
      const playerIds = getPlayerIds(match);
      if (playerIds.size === 0) return;

      // Find first court where all players are available
      ...
      for (let courtNum = 1; courtNum <= courtsCount; courtNum++) {
        const courtHasConflict = Array.from(playerIds).some(playerId => 
          courtPlayers[courtNum].has(playerId)
        );
      ...
      // If no court available, use court 1 (matches will queue)
      // This indicates too many simultaneous matches for available courts
      if (assignedCourt === null) {
        assignedCourt = 1;
      }

      match.court = assignedCourt;
      playerIds.forEach(playerId => courtPlayers[assignedCourt].add(playerId));
    });
  });
