    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      {stats.map((stat, idx) => (
        <div
          ...
          } ${stat.onClick ? "cursor-pointer ring-2 ring-yellow-400/30 hover:ring-yellow-300" : ""} ${
            stat.label === "Division" ? "h-44" : "h-32"
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <div className="flex space-x-1">
                {Array.from({ length: stat.streak }).map((_, i) => (
                  <div key={i} className="w-4 h-4 bg-green-500" title={`Win ${i + 1}`} />
                ))}
              </div>
            </div>
          )}
