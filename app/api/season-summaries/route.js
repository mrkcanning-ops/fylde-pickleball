...
export async function POST(request) {
  try {
    ...
    const payload = { ...body, timestamp: body.timestamp || new Date().toISOString() };
    ...
    return NextResponse.json({ data: Array.isArray(data) ? data.map(d => ({ ...d, id: d.id ? String(d.id) : d.id })) : [] });
  } catch (e) {
    return NextResponse.json({ error: e?.message || String(e) }, { status: 500 });
  }
}
